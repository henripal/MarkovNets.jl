...
"""
function Base.rand(mn::MarkovNet, sampler::MRFGibbsSampler, nsamples::Integer)
...
function gibbs_sample(mn::MarkovNet, nsamples::Integer, burn_in::Integer;
    ...
    mn_reduced = evidence_reduce(mrf, evidence)
...
function gibbs_sample_loop(mn::MarkovNet, current_sample::Assignment,
    ...
    for v in vertices(mn.ug)
        ...
        new_f = reduce(*, [mn.factors[factor_index][other_assg] for factor_index in mrf.name_to_factor_indices[Symbol(v)]])
